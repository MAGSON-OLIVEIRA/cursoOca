...
public class Agenda {
	...
	private Map<Character, List<Contato>> contatosPerLetrasMap = new TreeMap<Character, List<Contato>>();
	...
	public void inserir(Contato contato) throws AgendaException, IOException {
		...
		List<Contato> contatosLetra = contatosPerLetrasMap.get(letraInicial);
		if(contatosLetra == null) {
			contatosLetra = new ArrayList<Contato>();
			contatosPerLetrasMap.put(letraInicial, contatosLetra);
		}
	...
	public void excluir(String nome) throws AgendaException, IOException {
		...
		List<Contato> contatos = contatosPerLetrasMap.get(nome.toUpperCase().charAt(0));
		contatos.remove(contato);
		
		if(contatos.size() == 0) {
			contatosPerLetrasMap.remove(nome.toUpperCase().charAt(0));
		}
		arquivo.gravar(contatosMap.values());
	}
	...
	public List<Contato> listrContatosPorLetra(char letra){
		List<Contato> contatos = contatosPerLetrasMap.get(Character.toUpperCase(letra));
		if(contatos ==null) {
	...
	public List<Contato> listarContatosPorParteNome(String parteNome){
		...
		Pattern p = Pattern.compile(regex, Pattern.CASE_INSENSITIVE); // CASE_INSENSITIVE garante comparação desconsiderando maiúscula.
